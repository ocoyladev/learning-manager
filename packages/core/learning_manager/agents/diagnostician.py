...
class Diagnostician:
    ...
    def generate(
        ...
        result = complete_json(
            ...
            self._trajectory,
            "Diagnostician",
            "Create diagnostic multiple-choice questions.",
            f"Goal {goal.title}; concepts: {self._concept_ids}; create {n_items} items covering every concept.",
            ItemDraft,
        )
        return result.items
    ...
    def grade(
        ...
        for item in items:
            by.setdefault(item.concept_id, []).append(answers.get(item.id) == item.expected)
        states = {}
        for cid in self._concept_ids:
            ...
            states[cid] = LearnerConceptState(
                ...
                mastery=mastery,
                confidence=1.0,
                state=ConceptState.MASTERED if mastery >= 0.85 else ConceptState.WEAK,
                last_assessed=today,
            )
