...
def _topics(path: Path) -> dict[str, list[str]]:
    ...
    for raw in path.read_text().splitlines():
        ...
        result[topic.strip()] = [v.strip().strip("'\"") for v in values.strip().strip("[]").split(",") if v.strip()]
    return result
...
def build_corpus(topics_file: Path, corpus_path: Path = Path("corpus")) -> None:
    ...
    for topic, urls in _topics(topics_file).items():
        ...
        with httpx.Client(timeout=15, follow_redirects=True, headers={"User-Agent": "LearningManager/0.1"}) as client:
            for index, url in enumerate(urls, start=1):
                response = client.get(url)
                response.raise_for_status()
                html = response.text
                title = LiveSearchProvider._title(html) or urlparse(url).path.rsplit("/", 1)[-1] or url
                ...
                manifest.append({"id": source_id, "url": url, "title": title, "authority": classify_authority(url, title).value, "version": extract_version(html, title), "published_at": published.isoformat() if published else None, "retrieved_at": captured, "content_path": f"{topic}/{filename}"})
